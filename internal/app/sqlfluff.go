...
func NewSQLFluff(dir string, msgs chan Msg) (*App, error) {
	// Check if sqlfluff is installed
	_, err := exec.LookPath("sqlfluff")
	if err != nil {
		return nil, fmt.Errorf("'sqlfluff' not found in PATH")
	}
...
func (a *SQLFluff) Name() string {
	return "sqlfluff"
}

func (a *SQLFluff) Color() string {
	return "#a6e3a1"
}

func (a *SQLFluff) Success() *bool {
	return a.success
}

func (a *SQLFluff) Start(ctx context.Context) {
	a.wg.Wait()
...
func (a *SQLFluff) watch(ctx context.Context) {
	a.wg.Add(1)
	defer a.wg.Done()
...
func (a *SQLFluff) lint() error {
	...
	// Run revive
	cmd := exec.Command("sqlfluff", "lint")
	cmd.Dir = a.dir
